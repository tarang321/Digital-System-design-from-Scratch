...
module tb_carry_reduction6x6;
    ...
    // Instantiate the carry_reduction6x6 module
    carry_reduction6x6 uut (
        .A(A),
        .B(B),
        .p(p)
    );
    ...
    initial begin
        // Monitor A, B, and p (product) at every time unit
        $monitor("Time: %0t, A: %b, B: %b, Product: %b", $time, A, B, p);
    end

    // Initialize the simulation and create a waveform for viewing
    initial begin
        $dumpfile("tb_carry_reduction6x6.vcd");
        $dumpvars(0, tb_carry_reduction6x6);
    end
